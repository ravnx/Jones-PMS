...
public class PersonComboBox extends JComboBox<String>{
	...
	// Removed unused comboBoxModel field
	...
	public Person getSelectedPerson() {
		...
		if (currentPersons.isEmpty()) {
			// The list is empty, return null or handle appropriately
			return null;
		}
		if (selectedIndex < 0 || selectedIndex >= currentPersons.size()) {
			// No valid selection, return null or handle appropriately
			return null;
		}
		return currentPersons.get(selectedIndex);
	}
	...
	public void setPersonList(ArrayList<Person> personList) {
		...
		// Populate currentPersons with all persons initially.
		currentPersons = new ArrayList<>(personList);
		removeAllItems();
